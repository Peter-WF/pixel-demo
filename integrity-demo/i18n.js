(() => {
  ...
  style.textContent=`
    ...
    .bi-howto{margin-bottom:18px}
    .bi-howgrid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}
    .bi-step{background:var(--panel2);padding:13px;border-radius:10px;min-height:96px}
    .bi-stepno{font-size:10px;font-weight:800;color:#2563eb}
    .bi-steptitle{font-size:13px;font-weight:750;margin-top:6px}
    .bi-stepdesc{font-size:11px;color:var(--muted);line-height:1.5;margin-top:5px}
    ...
    body.shot .bi-howto{display:none}
    @media(max-width:1000px){.bi-howgrid{grid-template-columns:repeat(2,1fr)}}
    @media(max-width:700px){.bi-howgrid{grid-template-columns:1fr}}
  `;
  ...
  if(status&&!document.getElementById('biHowto')){
    ...
    b.className='card bi-howto';b.id='biHowto';
    status.parentNode.insertBefore(b,status);
  }

  function renderGuide(){
    const d=T[lang],b=document.getElementById('biHowto');
    if(!b)return;
    b.innerHTML=`<div class="section-title">${d.howTitle}</div><div class="bi-howgrid">${d.steps.map(s=>`<div class="bi-step"><div class="bi-stepno">${s[0]}</div><div class="bi-steptitle">${s[1]}</div><div class="bi-stepdesc">${s[2]}</div></div>`).join('')}</div><div class="bi-note">${d.note}</div>`;
  }
  ...
  function apply(){
    ...
    if(summaryCard){
      const st=summaryCard.querySelector('.section-title');if(st)st.textContent=d.summary;
      const ls=summaryCard.querySelectorAll('.label');
      [d.flags,d.total,d.max,d.weight].forEach((v,i)=>{if(ls[i])ls[i].textContent=v});
    }
    ...
    window.AppLovinEvidence?.enhanceTable?.();
    applying=false;
  }
